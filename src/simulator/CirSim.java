...
//	setupList = new Vector();
	...
	public void updateCircuit(Graphics realg) {
		 ...
		 * mouseElm.y); }
		 */
		if (getDragElm() != null && (getDragElm().getX() != getDragElm().getX2() || getDragElm().getY() != getDragElm().getY2()))
	...
	String getHint() {
		...
		if (hintType == HINT_LC) {
			...
			return "res.f = " + CircuitElm.getUnitText(1 / (2 * pi * Math.sqrt(ie.getInductance() * ce.getCapacitance())), "Hz");
		}
		if (hintType == HINT_RC) {
...
//		 ImportExportDialog.Action.EXPORT);
	...
	void dragPost(int x, int y) {
		if (draggingPost == -1) {
			draggingPost = (distanceSq(getMouseElm().getX(), getMouseElm().getY(), x, y) > distanceSq(getMouseElm().getX2(),
					getMouseElm().getY2(), x, y)) ? 1 : 0;
		}
		int dx = x - dragX;
		int dy = y - dragY;
		if (dx == 0 && dy == 0)
